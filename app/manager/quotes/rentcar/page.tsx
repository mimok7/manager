                    ...
                    {(() => {
                        ...
                        if (detailedServices.rentcars?.length) {
                            detailedServices.rentcars.forEach((rc: any) => {
                                ...
                                out += `렌트카(${p?.rent_category || ''})\n`;
                                out += `경로: ${p?.rent_route || ''}\n`;
                                out += `차량: ${p?.rent_car_type || ''}\n`;
                                        ...
                                        <div key={i} className="p-2 border rounded bg-white">
                                            ...
                                            {rc.priceInfo?.length > 0 && rc.priceInfo.map((p: any, pi: number) => (
                                                <div key={pi} className="mt-1 p-2 bg-gray-50 rounded">
                                                    <div className="text-sm">{p.rent_category ? `카테고리: ${p.rent_category}` : ''} {p.rent_route ? ` / 경로: ${p.rent_route}` : ''}</div>
                                                    <div className="text-sm">{p.rent_car_type ? `차량 타입: ${p.rent_car_type}` : ''}</div>
