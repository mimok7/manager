            ...
            <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              {modalView === 'reservation' && selectedReservation && (
                <>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusColor(selectedReservation.re_status)}`}>
                  ...
                  <div className="p-6">
                    {(() => {
                      const sd: any = selectedReservation.serviceDetails || {};
                      let dateStr = '';
                      let timeStr = '';
                      let locStr = '';
                      let details = '';

                      if (selectedReservation.re_type === 'cruise') {
                        if (sd.checkin) {
                          dateStr = new Date(sd.checkin).toLocaleDateString('ko-KR');
                        }
                        locStr = '하롱베이';
                        details = `${sd.guest_count || 0}명 | ${sd.room_price_code || ''}`;
                      } else if (selectedReservation.re_type === 'airport') {
                        if (sd.ra_datetime) {
                          const d = new Date(sd.ra_datetime);
                          dateStr = d.toLocaleDateString('ko-KR');
                          timeStr = d.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false });
                        ...
                        locStr = sd.ra_airport_location || '';
                        details = sd.ra_flight_number || '';
                      } else if (selectedReservation.re_type === 'hotel') {
                        if (sd.checkin_date) {
                          dateStr = new Date(sd.checkin_date).toLocaleDateString('ko-KR');
                        }
                        locStr = sd.hotel_category || '';
                        details = `${sd.nights || 0}박 | ${sd.guest_count || 0}명`;
                      } else if (selectedReservation.re_type === 'rentcar') {
                        if (sd.pickup_datetime) {
                          const d = new Date(sd.pickup_datetime);
                          dateStr = d.toLocaleDateString('ko-KR');
                          timeStr = d.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit', hour12: false });
                        ...
                        details = `${sd.rental_days || 0}일`;
                      } else if (selectedReservation.re_type === 'tour') {
                        if (sd.tour_date) {
                          dateStr = new Date(sd.tour_date).toLocaleDateString('ko-KR');
                        }
                        locStr = sd.pickup_location || sd.dropoff_location || '';
                        ...
                        <div className="space-y-4">
                          <div className="grid md:grid-cols-2 gap-4">
                            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                              <div className="text-sm text-gray-600">예약 ID</div>
                              <div className="font-mono text-gray-800">{selectedReservation.re_id}</div>
                            </div>
                            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                              <div className="text-sm text-gray-600">예약일</div>
                              <div className="text-gray-800">{new Date(selectedReservation.re_created_at).toLocaleString('ko-KR')}</div>
                            </div>
                          </div>

                          <div className="bg-white border border-gray-200 rounded-lg p-4">
                            <div className="text-sm text-gray-600 mb-2">일정/위치/세부</div>
                            <div className="text-gray-800 space-y-1">
                              {(dateStr || timeStr) && <div>📅 {dateStr}{timeStr ? ` ${timeStr}` : ''}</div>}
                              {locStr && <div>📍 {locStr}</div>}
                              {details && <div>ℹ️ {details}</div>}
                            </div>
                          </div>
                          ...
                          {selectedReservation.quote && (
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                              <div className="text-sm text-blue-700">연결된 견적</div>
                              <div className="text-blue-900 font-medium">{selectedReservation.quote.title}</div>
                            </div>
                          )}
