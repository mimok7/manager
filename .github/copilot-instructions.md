...
### 🤖 Auto 모드 빌드 및 배포 지침 (2025.11.21 추가)
- **빌드 및 Git 업로드 작업 시 모델 선택 규칙**:
  - ❌ **유료 모델(Claude Sonnet) 사용 금지**: 빌드 및 Git 푸시는 단순 반복 작업
  - ✅ **무료 0x 모델 사용 권장**: `o1-mini`, `o3-mini` 등 빠른 처리 모델 우선
  - ✅ **명령어 자동 실행**: `npm run build` → `git add .` → `git commit` → `git push`
  - 📝 **커밋 메시지**: 변경 내용을 간결하게 요약 (예: `feat: add sync page`, `fix: update schema`)
- **적용 시점**: 사용자가 "빌드하고 깃 업로드" 또는 유사한 요청을 할 때
- **목적**: 비용 절감 및 빠른 배포 처리
